    ...
    public void run() {
        ...
        for (int y = 0; y < yMultiplier; y++) {
            for (int x = min.getBlockX()-1; x <= max.getBlockX(); x++) {
                plugin.getWorkloadThread().add(new PlaceableBlock(Bukkit.getWorlds().get(0).getUID(),x,game.getCurrentLavaLevel(),min.getBlockZ()-1, Material.BARRIER));
                plugin.getWorkloadThread().add(new PlaceableBlock(Bukkit.getWorlds().get(0).getUID(),x,game.getCurrentLavaLevel(),max.getBlockZ(), Material.BARRIER));
            }
            for (int z = min.getBlockZ()-1; z <= max.getBlockZ(); z++) {
                plugin.getWorkloadThread().add(new PlaceableBlock(Bukkit.getWorlds().get(0).getUID(),min.getBlockX()-1,game.getCurrentLavaLevel(),z, Material.BARRIER));
                plugin.getWorkloadThread().add(new PlaceableBlock(Bukkit.getWorlds().get(0).getUID(),max.getBlockX(),game.getCurrentLavaLevel(),z, Material.BARRIER));
            }
            for (int x = min.getBlockX(); x < max.getBlockX(); x++) {
                for (int z = min.getBlockZ(); z < max.getBlockZ(); z++) {
                    plugin.getWorkloadThread().add(new PlaceableBlock(Bukkit.getWorlds().get(0).getUID(),x,game.getCurrentLavaLevel(),z, Material.LAVA));
                }
            }
